...
def live_strategy():
    ...
    # Wait until 9:15 if the current time is before 9:15
    if current_time < dt.time(9, 15):
        st.write("Waiting until 9:15 to start collecting prices...")
        while dt.datetime.now().time() < dt.time(9, 15):
            time.sleep(1)

    # Get high and low of BANKNIFTY index between 9:15 and 9:19 AM
    ...
    # Compute high and low after 9:19
    if prices:
        high = max(price[0] for price in prices)
        low = min(price[1] for price in prices)
        st.write("Final High:", high)
        st.write("Final Low:", low)
    ...
    if ce_high is None or pe_high is None:
        st.write("Failed to fetch option high prices.")
        return
    ...
    while not trade_executed and dt.datetime.now().time() > dt.time(9, 20):
        ...
        if ce_live_price and ce_live_price >= ce_high + 5:
            st.write(f"CE Option triggered at {ce_live_price}")
            place_order(f"BANKNIFTY{ce_strike_price}CE", ce_strike_price, "BUY")
            trade_executed = True
            break

        if pe_live_price and pe_live_price >= pe_high + 5:
            st.write(f"PE Option triggered at {pe_live_price}")   
            place_order(f"BANKNIFTY{pe_strike_price}PE", pe_strike_price, "BUY")
            trade_executed = True
            break
...
st.title('Live Trading Strategy')
if st.button('Run Strategy'):
    st.write("Strategy is running...")
    live_strategy()
